# Original file https://github.com/glejeune/cmake-
# samples/blob/master/sample8/cmake/Maven.cmake
set(MAVEN_DEPENDENCY_PLUGIN_VERSION "2.4")

include(FindJava)

if(${CMAKE_SYSTEM_NAME} MATCHES "Windows")
  find_program(MAVEN mvn.bat)
else()
...
# Usage :
#
# create_zip(name ARCHIVE file.zip FILES file ... [WORKING_DIRECTORY path]
# [DEPENDS depend ...] )
function(create_zip ZIP_TARGET)
  ...
  if(Java_JAR_EXECUTABLE)
    cmake_parse_arguments(ZIP "ARCHIVE;FILES;DEPENDS;WORKING_DIRECTORY;" ""
                          ${ARGN})
    ...
    if(NOT TARGET ${ZIP_TARGET})
      add_custom_target(${ZIP_TARGET} DEPENDS ${ZIP_DEPENDS})
    endif()
...
# usage :
#
# maven_deploy_file(target FILE name GROUP_ID groupId ARTIFACT_ID artifactId
# VERSION version PACKAGING zip|jar [REPOSITORY_ID repositoryId] [URL
# repositoryUrl] [DEPENDS depends ...] )
#
# If repositoryId is not given, this function will use
# MAVEN_DEFAULT_REPOSITORY_ID If repositoryUrl is not given, this function will
# use MAVEN_DEFAULT_URL
function(maven_deploy_file MAVEN_TARGET)
  ...
  if(MAVEN)
    set(options)
    set(oneValueArgs FILE GROUP_ID ARTIFACT_ID VERSION PACKAGING REPOSITORY_ID URL)
    ...
    if(NOT MAVEN_URL)
      if(NOT MAVEN_DEFAULT_URL)
        message(
          FATAL_ERROR "Repository URL not specified for maven_deploy_file")
      else()
    ...
    if(NOT TARGET ${MAVEN_TARGET})
      add_custom_target(${MAVEN_TARGET} DEPENDS ${MAVEN_DEPENDS})
    endif()
    ...
    add_custom_command(
      ...
      POST_BUILD
      COMMAND
        "${MAVEN}" "deploy:deploy-file" "-DgroupId=${MAVEN_GROUP_ID}"
        "-DartifactId=${MAVEN_ARTIFACT_ID}" "-Dversion=${MAVEN_VERSION}"
        "-Dpackaging=${MAVEN_PACKAGING}" "-Dfile=${MAVEN_FILE}"
        "-DrepositoryId=${MAVEN_REPOSITORY_ID}" "-Durl=${MAVEN_URL}")
  else()
...
#
# usage : maven_get_dependency( GROUP_ID net.java.dev.jna ARTIFACT_ID jna
# VERSION 3.4.0 [PACKAGING jar] [URL
# http://nexus.vidal.net:8081/nexus/content/groups/vidal-releases] [DESTINATION
# /path/to/jna-3.4.0.jar] [DEPENDS depends ...] [TARGET target] )
#
# If repositoryId is not given, this function will use
# MAVEN_DEFAULT_REPOSITORY_ID If repositoryUrl is not given, this function will
# use MAVEN_DEFAULT_URL
function(maven_get_dependency)
  ...
  if(MAVEN)
  set(options)
  set(oneValueArgs TARGET GROUP_ID ARTIFACT_ID VERSION PACKAGING DESTINATION URL)
  set(multiValueArgs DEPENDS)
  ...
  cmake_parse_arguments(
    ...
    if(NOT MAVEN_REPOSITORY_ID)
      if(NOT MAVEN_DEFAULT_REPOSITORY_ID)
        message(
          FATAL_ERROR "Repository ID not specified for maven_get_dependency")
      else()
        set(MAVEN_REPOSITORY_ID ${MAVEN_DEFAULT_REPOSITORY_ID})
      endif()
    endif()
    if(NOT MAVEN_URL)
      if(NOT MAVEN_DEFAULT_URL)
        message(
          FATAL_ERROR "Repository URL not specified for maven_get_dependency")
      else()
        set(MAVEN_URL ${MAVEN_DEFAULT_URL})
      endif()
    endif()
    ...
    if(MAVEN_TARGET)
      add_custom_command(
        ...
        COMMAND
          ...
          "-DgroupId=${MAVEN_GROUP_ID}" "-DartifactId=${MAVEN_ARTIFACT_ID}"
          "-Dversion=${MAVEN_VERSION}" "-Dpackaging=${MAVEN_PACKAGING}"
          "-DremoteRepositories=${MAVEN_URL}" ${MAVEN_DESTINATION})
    else()
      message(
        STATUS
          "Get maven dependency ${MAVEN_GROUP_ID}:${MAVEN_ARTIFACT_ID}:${MAVEN_VERSION}:${MAVEN_PACKAGING}"
      )
      execute_process(
        COMMAND
          "${MAVEN}"
